  ...
  return (
    <div className="dialog-backdrop" role="presentation" onMouseDown={onClose}>
      <section className="config-dialog" role="dialog" aria-modal="true" aria-labelledby="config-title" onMouseDown={(event) => event.stopPropagation()}>
        <header><div><span>QWEN AGENTS + WAN LORA</span><h2 id="config-title">连接阿里云百炼</h2></div><button type="button" aria-label="关闭" onClick={onClose}><Icon name="x" size={17} /></button></header>
...
WAN_DEPLOYED_MODEL=部署成功后返回的_deployed_model
WAN_LORA_TRIGGER_WORD=t64px9`}</pre>
